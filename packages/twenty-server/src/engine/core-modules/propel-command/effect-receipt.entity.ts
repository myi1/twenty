...
// Durable counterpart of CommandReceiptEntity. One row per commandId: the unique
// index is the claim, so two callers racing on the same id can never both win a
// fresh claim, and a resumed id finds the row the crashed attempt left behind.
@Entity({ name: 'effect_receipt', schema: 'core' })
@Index('IDX_EFFECT_RECEIPT_COMMAND_ID', ['commandId'], { unique: true })
