...
export const serializeEvent = (event: unknown): SerializedEventData => {
  ...
  // Propel fork — drag-and-drop: dropped files live on event.dataTransfer.files,
  // not on a form target. Surface them through the same `files` channel so a
  // component reads e.target.files uniformly across the file-input and drop paths.
