...
// Media Studio (LP Builder v2 §4.3 / §5) — the full-screen image workbench that
// REPLACES the cramped ProjectImagePicker popover. One modal, opened by every
// image field's "Change" button, with a top-tab source nav and a 2-column body:
//
//   Library (default) · Generate · Enhance · Project renders · Upload(soon)
//
...
// the user clicks "Save to library" — a button explicitly separate from "Use this
// image" (which inserts the gatewayPath into the field and closes).
//
...
//  • transient error     → toast; the modal stays open.
...
export const MediaStudioModal = ({
  ...
  // inline source picker), while the modal is open.
  useEffect(() => {
    const needsLibrary = opened && (tab === 'library' || (tab === 'enhance' && enhPickerOpen));
    if (needsLibrary && !libraryLoaded && !libraryBusy) {
      void loadLibrary();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [opened, tab, enhPickerOpen, libraryLoaded, libraryBusy]);
  ...
  const libraryGrid = (onTile: (a: WebsiteAsset) => void) => (
      ...
      <Group align="flex-start" gap="xl" wrap="nowrap" style={{ minHeight: 0 }}>
        ...
        <Stack gap="sm" style={{ flex: 1, minWidth: 0 }}>
          ...
          <Group gap="xs" justify="space-between">
            ...
            {useAndSave(genResult, genSaving, genSaved, () => void saveGeneratedToLibrary())}
          </Group>
        </Stack>
      </Group>
    );
      ...
      <Group align="flex-start" gap="xl" wrap="nowrap" style={{ minHeight: 0 }}>
        ...
        <Stack gap="sm" style={{ flex: 1, minWidth: 0 }}>
          ...
          <Group justify="flex-end">
            {useAndSave(enhResult, enhSaving, enhSaved, () => void saveEnhancedToLibrary())}
          </Group>
        </Stack>
      </Group>
    );
