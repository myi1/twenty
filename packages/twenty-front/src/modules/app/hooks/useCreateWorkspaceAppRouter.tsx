        ...
        <Route element={<MinimalMetadataGate />}>
          <Route element={<DefaultLayout />}>
            <Route element={<MainAppLayoutWithSidePanel />}>
              ...
              />
              {/* Propel hero routes, config-driven. All heroes are runtime-
                  loaded via <HeroRoute name="…"/> — none ship in the app bundle.
                  Routes are built from the nav config available synchronously at
                  router-construction time (the baked default lists every known
                  hero); they register UNCONDITIONALLY (enabled or not) so a
                  nav-hidden entry still resolves for deep-links. Re-ported from
                  the pre-2.14 useCreateAppRouter into the workspace router of the
                  upstream root/workspace split. */}
              ...
              {/* Catch-all hero route — closes the "new hero needs a rebuild"
                  gap: a hero added ONLY to the host-mounted nav.config.json uses
                  route '/h/<bundle>' and HeroCatchAllRoute resolves it AT
                  NAVIGATION TIME, no rebuild. Registered AFTER the explicit hero
                  routes (those win by specificity) and BEFORE the NotFound
                  wildcard (so heroes beat the 404). :bundle is sanitized in
                  HeroCatchAllRoute. */}
              <Route
